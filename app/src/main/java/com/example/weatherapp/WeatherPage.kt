    ...
    Column (
        ...
        Card {
            Row (modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceAround) {
                WeatherKeyVal(data.current.humidity, "Humidity")
                WeatherKeyVal(data = data.current.wind_kph + "km/h", label = "Wind Speed")
            }
            Row (modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceAround) {
                WeatherKeyVal(data.current.uv, "UV")
                WeatherKeyVal(data = data.current.precip_in, label = "Precipitation")
            }
            Row (modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceAround) {
                WeatherKeyVal(data.location.localtime.split(" ")[1], "Local time")
                WeatherKeyVal(data.location.localtime.split(" ")[0], label = "Local Date")
            }
        }
...
@Composable
fun WeatherKeyVal(data: String, label: String) {
    Column (
